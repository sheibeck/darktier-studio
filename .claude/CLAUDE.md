...
| `@astrojs/react` | 6.0.2 | React island integration | `include` now covers `**/admin/**`, `**/live/**` (the live public catalog islands), and `**/components/armory/**` (the in-site companion apps) — not `/admin` only. See "Stack Patterns by Variant" below — do **not** apply this globally beyond those globs. |
| `react` / `react-dom` | 19.2.8 | Admin editor UI, live catalog islands, and Armory companion apps | The source Design project's "DC" prototypes are React; reusing React for the authenticated `/admin` route, the live-catalog islands, and the code-split `/armory/<slug>` companion apps lets you carry over interaction patterns from those prototypes without shipping any of that JS to plain marketing pages (Home/Games) — those still ship ~0KB app JS since each island/app is code-split to its own route/component. |
...
| A general-purpose CSS framework (Tailwind, Bootstrap) or CSS-in-JS layered on top of Nocturne | The design is already approved as a single hand-authored `styles.css` of CSS custom properties + component classes; layering a utility framework on top invites token/spec drift and fights the "reuse verbatim" constraint | Import `styles.css` as-is as a global stylesheet in Astro's base layout; add page-specific `<style>` blocks (scoped, Astro-native) only for truly one-off layout needs, using existing CSS variables. **Scoped exception (INTENTIONAL, isolated):** Tailwind v4 is compiled for the Burning Banners armory companion app only (`src/styles/armory-bb-tailwind.css`), imported only from that one component, with `preflight` never generated and never imported by `Layout.astro`, `nocturne.css`, or `site.css` — no Nocturne page is affected. This does not license adding Tailwind anywhere else; the "don't layer a utility framework over Nocturne" guidance still stands outside this one scoped island. See `docs/adding-a-companion-app.md` for the pattern to follow if another companion app needs it. |
